<template>
  ...
  >
    <div
        ...
        @touchend.passive
      >
      <div class="card-image-container" @touchstart.passive @touchmove.passive @touchend.passive>
...
function enforceInlineAutoplay(v: HTMLVideoElement) {
  try {
    v.muted = true
...
function onTouchMove(e: TouchEvent) {
  ...
  lastTouchX.value = t.clientX
  lastTouchY.value = t.clientY
  // Determine gesture axis early and once
  const dx = Math.abs(lastTouchX.value - touchStartX.value)
  const dy = Math.abs(lastTouchY.value - touchStartY.value)
  // If vertical movement dominates, mark and let the browser handle scrolling
  if (!isVerticalGesture.value && dy > dx && dy > 4) {
    isVerticalGesture.value = true
  }
}
function onTouchEnd() {
  if (!isTouching.value) return
  const wasVertical = isVerticalGesture.value
  isTouching.value = false
  isVerticalGesture.value = false
  if (wasVertical) {
    // It was a vertical scroll gesture; don't snap horizontally
    onUserInteraction()
    return
  }
  const w = getPageWidthPx()
...
onMounted(async () => {
  ...
  observer = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      ...
      if (entry.isIntersecting) {
        // Ensure inline autoplay on mobile
        enforceInlineAutoplay(v)
      } else {
        v.pause()
      }
    })
  ...
  // Try to start all immediately as well (may be blocked until first gesture)
  videoRefs.value.forEach((v) => enforceInlineAutoplay(v))

  // As a fallback for iOS autoplay policy, attempt resume on first user interaction
  const resumeAll = () => {
    videoRefs.value.forEach((v) => enforceInlineAutoplay(v))
    window.removeEventListener('touchstart', resumeAll, { passive: true } as any)
    window.removeEventListener('click', resumeAll)
  }
  window.addEventListener('touchstart', resumeAll, { passive: true } as any)
...
/* Hide native controls overlays on iOS/Safari where possible */
